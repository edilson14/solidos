...
def toroide(raio_origem_toroide, raio_abertura_toroide, quantidades_pontos_circulo=30, num_points_per_circle=100, origin=np.array([0, 0, 0])):
    ...
    for i in range(quantidades_pontos_circulo):
        ...
        for j in range(num_points_per_circle):
            ...
            point = circle_center + np.array([raio_abertura_toroide * np.cos(phi), raio_abertura_toroide * np.sin(phi), 0])
...
def plot_torus(vertices, edges):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    # Extrai as coordenadas x, y, z dos vértices
    x, y, z = zip(*vertices)

    # Plota os vértices e as arestas
    ax.scatter(x, y, z, c='r', marker='*') # Plota os vértices do toroide
    for edge in edges:
...
torus_vertices, torus_edges = toroide(R, r, num_circles, num_points_per_circle, origin)
plot_torus(torus_vertices, torus_edges)
